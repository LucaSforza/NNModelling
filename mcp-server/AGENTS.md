...
- `src/tools/` contains narrow MCP adapters for graph, parameters, selection,
  canvas, validation, package serialization, inspection, connection, lifecycle
  and optional remote-training actions.
...
- Package compilation and training belong to the authenticated backend API.
  The MCP server does not spawn Python conversion/training subprocesses or
  reconstruct NNTree data.
...
method names or payloads require corresponding frontend
`BrowserRPCHandler` tests.
