return function(context, parameters, services)
  ...
  if rank < 1 then
    return { status = "error", message = "KL Divergence expects packed Gaussian parameters with rank at least 1" }
  end
  ...
  if input_dtype ~= "float16"
    ...
    return {
      status = "error",
      message = "KL Divergence expects a floating input dtype, got " .. input_dtype
    }
  end
  ...
  if type(packed_size) ~= "number" then
    return { status = "error", message = "KL Divergence requires a numeric packed last dimension" }
  end
  if packed_size % 2 ~= 0 then
    return { status = "error", message = "KL Divergence requires an even packed last dimension" }
  end
